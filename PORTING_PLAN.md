...
| **Database & Migrations** | `sql/migrations/` (`YYYYMMDDHHMMSS_world.sql`), 1.12.1 vanilla schema with progressive versioning columns (`patch`, `build`). | `sql/database_updates/world/`, Nostalrius schema, custom tables (`tw_world_*`), custom spell IDs (>= 40000) and world IDs (>= 300000). | **Medium** | `Patch 1.18.1 - Itemization Changelog`, `1.17.0/1.17.1 Itemization Changelogs` | Convert migrations to Tortoise format; scalp and diff vanilla entities via `task scalp` from Brotalnia `world_full_14_june_2021.sql` (Main Historic DB), `vmangos/core db_latest` (Backup DB) & `tortoise-db-viewer` (Dashboard & API); verify tooltips via DB Viewer (`task 6` / `task dashboard`); strip progressive columns; protect custom IDs; audit via `Audit-DatabaseMigrations.ps1`. |
